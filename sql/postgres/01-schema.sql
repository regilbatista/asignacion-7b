-- PostgreSQL Schema para Unipago - CORREGIDO
-- Este archivo reemplaza el sql/postgres/01-schema.sql

CREATE SCHEMA IF NOT EXISTS payments;
SET search_path TO payments, public;

-- ============================================
-- CREAR TABLAS COMPLETAS CON TODAS LAS COLUMNAS
-- ============================================

-- Tabla de afiliados importados (COMPLETA)
...
-- Tabla de registros de pagos (COMPLETA)
...
-- Tabla de auditoría de importaciones (COMPLETA)
...
-- Tabla de auditoría de afiliados (COMPLETA)
...
BEGIN
    ...
    RAISE NOTICE 'Tablas creadas en schema payments: %', table_count;
    
    -- Mostrar las tablas creadas
    FOR table_count IN 
        SELECT COUNT(*) FROM affiliates
    LOOP
        RAISE NOTICE 'Tabla affiliates lista (% registros)', table_count;
    END LOOP;
END $$;